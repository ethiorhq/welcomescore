...
Use a Supabase publishable or anon key only. Do not use `SUPABASE_SERVICE_ROLE_KEY` in a `NEXT_PUBLIC_` variable. Lounge messages are limited to 300 characters, locally filtered for basic spam/profanity, throttled to one send every three seconds per browser session, and automatically purged after 24 hours. Hover a message on desktop or swipe/long-press it on touch devices to reply; a quoted snapshot keeps the conversational context readable even after the original message expires.
