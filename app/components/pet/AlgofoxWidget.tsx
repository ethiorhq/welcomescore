...
export default function AlgofoxWidget() {
  ...
  return (
    <aside className="pointer-events-none fixed bottom-3 right-3 z-40 flex max-w-[min(16rem,calc(100vw-1.5rem))] flex-col items-end sm:bottom-5 sm:right-5" aria-label="Algofox guide">
      ...
      <button
        type="button"
        onClick={() => setAlgofoxState("waving", "Still here. Let’s make a repository easier to join.", 4_000)}
      ...
      >
        <AlgofoxSprite state={state} size={72} className="drop-shadow-[0_10px_20px_rgba(0,0,0,0.35)] sm:[transform:scale(1.1)] sm:origin-bottom-right" />
      </button>
    </aside>
  );
}
