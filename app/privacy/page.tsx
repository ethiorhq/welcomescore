...
const sections: PolicySection[] = [
    ...
    content: (
      <PolicyTable>
        ...
        <tbody className="divide-y divide-muted/15 text-muted">
          ...
          <tr><td className="px-4 py-3 font-mono text-text">Dev Lounge</td><td className="px-4 py-3">The anonymous message, quoted-reply snapshot, optional score card, reaction choice, temporary browser-generated developer handle/avatar, session-scoped identifiers, short-lived signed interaction proof, and salted network-rate bucket needed for temporary chat and abuse protection. The raw network identifier is not stored in the rate record.</td><td className="px-4 py-3">To provide a lightweight 24-hour community discussion experience and protect it against automated or excessive requests.</td></tr>
          <tr><td className="px-4 py-3 font-mono text-text">Lounge reports and moderation</td><td className="px-4 py-3">The reported message identifier, report category, optional limited detail, a private structured moderation decision, confidence band, policy category, provider/model label, and action timestamp. If AI review is configured, only the reported public message and report category are sent to that provider; the reporter identity, raw network data, unrelated chat history, and credentials are excluded.</td><td className="px-4 py-3">To review private safety reports, prevent clear high-severity abuse, and maintain a limited owner audit trail.</td></tr>
          <tr><td className="px-4 py-3 font-mono text-text">Visitor verification</td><td className="px-4 py-3">When enabled, a Cloudflare Turnstile challenge token is sent from the browser to WelcomeScore’s server for one-time validation. WelcomeScore does not store the token in the Lounge database.</td><td className="px-4 py-3">To distinguish ordinary visitor activity from automated form abuse.</td></tr>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>Dev Lounge messages and related temporary chat records are designed to expire after 24 hours. Quoted-reply snapshots may remain only for the same limited retention period. Private report, structured moderation, and salted rate-limit records are designed to expire after 30 days. Signed interaction proofs are short-lived and are not stored as a Lounge record. Browser-local identity details are stored in your browser and can generally be removed by clearing the site’s local storage or browser data.</p>
  ...
  },
  {
    ...
    content: (
      <>
        <p>WelcomeScore obtains public repository information from GitHub. The hosted application and permitted product data are operated through its deployment and database services. Optional repository reviews and event-driven private Lounge report reviews may use configured AI providers; the service sends only the constrained context described above and uses a stateless provider setting where supported. When configured, Cloudflare Turnstile processes visitor-verification challenges; its processing is governed by Cloudflare’s own terms and privacy disclosures.</p>
  ...
  },
  {
    ...
    content: (
      <>
        <p>No internet service can guarantee absolute security. WelcomeScore uses practical separation of browser-safe and server-only credentials, private review-cache permissions, signed interaction proofs, server-side visitor-verification checks when configured, and limited retention, but you should still avoid sharing sensitive information. Automated moderation is a safety support, not a guarantee that harmful material will be detected or removed. For a suspected vulnerability, follow <Link className="text-link underline underline-offset-4" href="https://github.com/ethiorhq/welcomescore/blob/main/SECURITY.md">SECURITY.md</Link> rather than posting publicly.</p>
