...
const sections: PolicySection[] = [
  ...
  {
    id: "js-health",
    title: "JavaScript Health and the local CLI",
    content: (
      <>
        <p>
          JavaScript Health is a separate, deterministic 100-point view for JavaScript and TypeScript maintainers. It checks observable package metadata, Node tooling, GitHub Actions signals, and contributor foundations. It does not replace the contributor-readiness audit, and its score does not certify code quality, package safety, security, legal compliance, popularity, or trustworthiness.
        </p>
        <p>
          The public dashboard reads an explicit public GitHub repository request and returns only an evidence summary. The <code className="font-mono text-text">npx @ethiorhq/welcomescore</code> CLI reads the project directory you choose locally. It does not upload source files, package manifests, environment values, or Git credentials to WelcomeScore. Its <code className="font-mono text-text">--fix</code> mode creates only absent low-risk templates and never overwrites a file, edits package metadata, installs dependencies, runs scripts, commits code, or publishes a package.
        </p>
        <PolicyCallout>
          Any remediation template is a maintainer-reviewed starting point. Verify that it matches your project, supported Node versions, package interface, license, contributor policy, and release process before using it.
        </PolicyCallout>
      </>
    ),
  },
